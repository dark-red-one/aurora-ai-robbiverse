/**
 * RobbieBar - Top bar for Cursor IDE (Robbie@Code)
...
export const RobbieBar: React.FC<RobbieBarProps> = ({ 
  ...
  const handleOpenChat = () => {
    logUser.action('Open chat clicked', { compact })
    
    if (onOpenChat) {
      onOpenChat()
    } else {
      // Default: open Robbie@Code in new window
      logSystem.event('Opening Robbie@Code in new window')
      window.open('http://aurora.testpilot.ai/code/', '_blank', 'width=800,height=900')
    }
  }

  const handleReload = () => {
    logUser.action('Reload clicked')
    setIsReloading(true)
    setTimeout(() => {
      logSystem.event('Reloading page...')
  ...
  const handleMoodClick = () => {
    ...
    logUser.action('Mood clicked', { currentMood })
    
    // Cycle through moods on click
    cycleMood()
    
    // Log will happen automatically via store subscription
  }
    ...
    <div className={`robbiebar ${compact ? 'robbiebar-compact' : ''}`}>
      ...
      <div className="robbiebar-content">
        ...
        >
          ...
          <div className="mood-info">
            ...
            <div className="mood-greeting">
              {compact ? "Let's code! 🚀" : greeting.split('\n')[0]}
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          title="Open Robbie@Code Full Interface"
        >
          💬 Chat
        </button>
        ...
        <div className="robbiebar-users">
          ...
          {activeUsers.map((user, index) => (
            <span 
              ...
              title={`${user} is coding${user === 'Allan' ? ' 🔥' : ''}`}
              style={{
