/**
 * RobbieBar - Top bar for Cursor IDE (Robbie@Work)
...
export const RobbieBar: React.FC<RobbieBarProps> = ({ 
  ...
  const handleOpenChat = () => {
    if (onOpenChat) {
      ...
      // Default: open Robbie@Work in new window
      window.open('http://aurora.testpilot.ai/work/', '_blank', 'width=800,height=900')
    }
  }
    ...
    <div className={`robbiebar ${compact ? 'robbiebar-compact' : ''}`}>
      ...
      <div className="robbiebar-content">
        ...
        >
          ...
          <div className="mood-info">
            ...
            <div className="mood-greeting">
              {compact ? "Let's work! 💼" : greeting.split('\n')[0]}
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          title="Open Robbie@Work Full Interface"
        >
          💬 Chat
        </button>
        ...
        <div className="robbiebar-users">
          ...
          {activeUsers.map((user, index) => (
            <span 
              ...
              title={`${user} is working${user === 'Allan' ? ' 🔥' : ''}`}
              style={{
