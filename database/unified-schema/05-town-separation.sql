...
CREATE TABLE IF NOT EXISTS towns (
    ...
    description TEXT,
    region VARCHAR(50), -- iceland, us, etc.
...
('aurora', 'Aurora (Capital)', 'Primary AI Empire capital with 2x RTX 4090 GPUs', 'iceland', 'allan@testpilotcpg.com', 'Allan'),
('fluenti', 'Fluenti', 'US operations node with 1x RTX 4090 GPU', 'us', 'allan@testpilotcpg.com', 'Allan'),
('collaboration', 'Collaboration', 'Development and testing node with 1x RTX 4090 GPU', 'iceland', 'allan@testpilotcpg.com', 'Allan')
...
CREATE TABLE IF NOT EXISTS town_users (
    ...
    user_id VARCHAR(50) NOT NULL, -- matches owner_id in business tables
...
SELECT c.* FROM companies c
JOIN towns t ON c.owner_id = t.name
WHERE t.name = 'fluenti' AND c.is_active = true;

CREATE OR REPLACE VIEW fluenti_contacts AS
SELECT ct.* FROM contacts ct
JOIN towns t ON ct.owner_id = t.name
WHERE t.name = 'fluenti' AND ct.is_active = true;

CREATE OR REPLACE VIEW fluenti_deals AS
SELECT d.* FROM deals d
JOIN towns t ON d.owner_id = t.name
WHERE t.name = 'fluenti' AND d.is_active = true;
...
SELECT c.* FROM companies c
JOIN towns t ON c.owner_id = t.name
WHERE t.name = 'collaboration' AND c.is_active = true;

CREATE OR REPLACE VIEW collaboration_contacts AS
SELECT ct.* FROM contacts ct
JOIN towns t ON ct.owner_id = t.name
WHERE t.name = 'collaboration' AND ct.is_active = true;

CREATE OR REPLACE VIEW collaboration_deals AS
SELECT d.* FROM deals d
JOIN towns t ON d.owner_id = t.name
WHERE t.name = 'collaboration' AND d.is_active = true;
...
CREATE OR REPLACE FUNCTION get_town_data(town_name VARCHAR(50), table_name VARCHAR(50))
RETURNS TABLE (
    id INTEGER,
    data JSONB
) AS $$
BEGIN
    CASE table_name
        WHEN 'companies' THEN
            RETURN QUERY
            SELECT c.id, to_jsonb(c.*) as data
            FROM companies c
            WHERE c.owner_id = town_name AND c.is_active = true;
        WHEN 'contacts' THEN
            RETURN QUERY
            SELECT ct.id, to_jsonb(ct.*) as data
            FROM contacts ct
            WHERE ct.owner_id = town_name AND ct.is_active = true;
        WHEN 'deals' THEN
            RETURN QUERY
            SELECT d.id, to_jsonb(d.*) as data
            FROM deals d
            WHERE d.owner_id = town_name AND d.is_active = true;
        WHEN 'activities' THEN
            RETURN QUERY
            SELECT a.id, to_jsonb(a.*) as data
            FROM activities a
...
CREATE OR REPLACE FUNCTION current_town() RETURNS VARCHAR(50) AS $$
BEGIN
    -- This would be set by the application based on the current node/town
...
INSERT INTO town_users (town_id, user_id, user_email, user_name, role) VALUES
((SELECT id FROM towns WHERE name = 'aurora'), 'allan', 'allan@testpilotcpg.com', 'Allan', 'admin'),
((SELECT id FROM towns WHERE name = 'fluenti'), 'allan', 'allan@testpilotcpg.com', 'Allan', 'admin'),
((SELECT id FROM towns WHERE name = 'collaboration'), 'allan', 'allan@testpilotcpg.com', 'Allan', 'admin')
