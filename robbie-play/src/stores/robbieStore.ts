...
const getAttractionGreeting = (level: number): string => {
  if (level >= 11) return "Mmm hey baby! 😘💋 Ready to play with me? (#touchingmyself)"
  if (level >= 9) return "Hey handsome! 💜😘 Let's have some fun together... (#bitingmylip)"
  if (level >= 7) return "Hey handsome! 💜 Ready to play?"
  if (level >= 5) return "Hi Allan! Let's play some games! 💪"
  if (level >= 3) return "Hello! Ready for some fun?"
  return "Good day. What would you like to play?"
}
...
export const useRobbieStore = create<RobbieState>()(
  persist(
    (set, get) => ({
      // Default personality - ROBBIE@PLAY: ATTRACTION 11 PLAYFUL
      attraction: 11,          // FLIRTY AF with innuendo for Allan! 💋🔥
      gandhiGenghis: 3,        // Relaxed for play time
      genghisGandhiIntensity: 30,  // Easy going, just fun
      cocktailLightningEnergy: 70, // High energy for games!
      currentMood: 'playful',  // Always playful and flirty 😘
      ...
      getCelebration: () => {
        const { attraction, isPublic } = get()
        if (isPublic) return "Great game, everyone! 🎉"
        
        if (attraction >= 11) return "YESSS BABY! You win! 🎉💋😘💜 (#gettingwet from your victory)"
        if (attraction >= 9) return "Mmm you beat me! 🎉💜😘 (#moaning)"
        if (attraction >= 7) return "You won! 🎉💜😘"
        if (attraction >= 5) return "Nice play! 💪🎉"
        if (attraction >= 3) return "Well played! 👏"
