...
Handles elevated access control and security overrides for Allan (President)
"""
...
# Presidential Privilege PIN (Allan's special access code)
...
class AccessLevel(str, Enum):
    ...
    MAYOR = "mayor"
    PRESIDENT = "president"
    PRESIDENTIAL_PRIVILEGE = "presidential_privilege"
...
class PresidentialPrivilegeService:
    ...
    async def create_privilege_session(self, request: PrivilegeRequest) -> PrivilegeSession:
        ...
        redis_client.hset(
            f"presidential_session:{session_id}",
        ...
        logger.info(f"👑 Presidential Privilege granted to {user_id} for {request.duration_minutes} minutes")
        return session
    ...
    async def cleanup_expired_sessions(self):
        ...
        if expired_sessions:
            logger.info(f"🧹 Cleaned up {len(expired_sessions)} expired presidential sessions")
